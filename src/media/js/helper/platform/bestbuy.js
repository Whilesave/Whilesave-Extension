($ => {
     ...
     * @param {*} ext 
     */
    $.BestbuyHelper = function (ext, support) {
        ...
        this.detailCouponAnalyze = function(result){
            ...
            if(templateIdEle){
                ...
                if(couponCodeElement){
                    const promoCode = util.helper.util.decryptStr(couponCodeElement.getAttribute("data-encryptcode"));
                    templateIdEle.addEventListener("click", ()=>{
                        ext.helper.util.setClipboard(promoCode).then(()=>{
                            ...
                            if(mid && mid.hasOwnProperty("target") && mid.hasOwnProperty("link") && mid.hasOwnProperty("delay")){
                                const {target, link, delay}  = mid, linkDecrypt = util.helper.util.decryptStr(link);
                                setTimeout(()=>{
                                if(target==="_blank"){
                                    util.helper.util.openInTab(linkDecrypt);
