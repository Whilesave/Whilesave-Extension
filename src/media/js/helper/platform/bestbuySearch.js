($ => {
     ...
     * @param {*} ext 
     */
    $.BestbuySearchHelper = function (ext, support) {
                    ...
                    if(elementObj.element){
                        ...
                        elements.forEach((element)=> {
                            if(element && ext.helper.elementUtil.isElementDisplayed(element) && !element.getAttribute($.attr.couponProcessMark)){
                                ...
                                const price = ext.helper.util.getGoodsPrice(
                                    ext.helper.util.getGoodsPriceByElement(element, priceQuery)
                                );
								//ext.logger("info", "search price======>", price);

                                let id = null;
                                if(this.isItemLink(goodsLink)){
                                    id = ext.helper.util.getGoodsIdByLink(goodsLink.getAttribute("href"),"p");
                                }
                                if(id){
                                    items.push({
